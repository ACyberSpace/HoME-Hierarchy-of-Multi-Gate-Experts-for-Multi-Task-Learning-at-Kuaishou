...
class KuairandDataset(Dataset):
    ...
    def __init__(
        ...
        self.mode = mode
        self.data = data[mode]
        self.short_seq_col = self._resolve_column(
            ["short_seq", "last_k_clicked_items"],
            required=True,
        )
        self.short_mask_col = self._resolve_column(
            ["short_mask", "short_seq_mask"],
            required=True,
        )
    ...
    def _resolve_column(self, candidates, required: bool = False):
        for name in candidates:
            if name in self.data:
                return name
        if required:
            raise KeyError(f"[{self.mode}] missing any of columns: {candidates}")
        return None
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        sample = {
            ...
            'labels': {},
            # ========== 新增：序列特征 ==========
            'short_seq': torch.tensor(self.data[self.short_seq_col][idx], dtype=torch.long),
            'short_seq_mask': torch.tensor(self.data[self.short_mask_col][idx], dtype=torch.float),

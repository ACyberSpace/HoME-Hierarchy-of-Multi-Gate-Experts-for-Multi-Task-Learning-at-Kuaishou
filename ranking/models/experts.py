...
class MetaExpertLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_gates = nn.ModuleDict({
            'shared': SelfGate(expert_dim, num_shared),
            'watch': SelfGate(expert_dim, num_category),
            'interact': SelfGate(expert_dim, num_category)
        })
    ...
    def forward(self, x_dict: dict) -> dict:
        ...
        self_shared_outputs = self.self_gates["shared"](shared_outputs.mean(dim=1), shared_outputs)
        ...
        for category in self.task_groups.keys():
            ...
            self_intra_all_outputs = self.self_gates[category](category_outputs.mean(dim=1), category_outputs)

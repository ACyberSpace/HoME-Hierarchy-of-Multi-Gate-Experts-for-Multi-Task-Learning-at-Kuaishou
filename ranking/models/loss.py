    ...
    def forward(
        ...
        for task_name in self.task_names:
            ...
            if task_group and task_group in self.group_log_vars:
                group_log_var = self.group_log_vars[task_group]
                group_weight = torch.exp(-group_log_var) / 2
            else:
                group_log_var = None
                group_weight = 1.0
            ...
            if group_log_var is not None:
                weighted_loss = weighted_loss + group_log_var / 2
...
class MultiTaskBCELoss(nn.Module):
    """Equal-weight multi-task BCE loss for baseline comparisons."""
    ...
    def forward(
        ...
        return {
            ...
            'task_losses': {k: v.item() for k, v in task_losses.items()},
            'task_weights': task_weights,
            'uncertainties': {name: 1.0 for name in self.task_names}
        }

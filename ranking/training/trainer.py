...
class HoMETrainer:
    ...
    def __init__(
        ...
        # 权重追踪
        self.weight_history = []
        self.best_checkpoint_path = Path(self.config.save_dir) / "best_model.pt"
    ...
    def train(self, num_epochs: Optional[int] = None) -> None:
        ...
        for epoch in range(num_epochs):
            ...
            if 'avg_gauc' in eval_metrics:
                print(f"  平均GAUC: {eval_metrics['avg_gauc']:.4f}")

            monitor_metric = eval_metrics.get('avg_gauc', eval_metrics['avg_auc'])
            ...
            if monitor_metric > best_metric:
                ...
                torch.save({
                    ...
                    'monitor': 'avg_gauc' if 'avg_gauc' in eval_metrics else 'avg_auc',
                    'eval_metrics': eval_metrics
                }, self.best_checkpoint_path)
                print(f"\n✓ 保存最佳模型 (monitor: {best_metric:.4f})")
                patience_counter = 0
            else:
                patience_counter += 1
    ...
    def load_best_checkpoint(self) -> bool:
        ...
        monitor = checkpoint.get('monitor', 'metric')
        best_metric = checkpoint.get('best_metric', None)
        if best_metric is None:
            print(f"\n已加载最佳模型 ({monitor})")
        else:
            print(f"\n已加载最佳模型 ({monitor}: {best_metric:.4f})")
        return True

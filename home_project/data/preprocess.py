...
def preprocess(input_path: Path, output_path: Path) -> dict:
    ...
    df_merged_with_last_20_clicked_items = compute_last_k_clicked_history(
        df_merged[["user_id", "video_id", "date", "time_ms", "is_click"]],
        k=20,
        pad_value=0,  # 填充的值为0
    )
    df_merged_with_last_50_clicked_items = compute_last_k_clicked_history(
        df_merged[["user_id", "video_id", "date", "time_ms", "is_click"]],
        k=20,
        pad_value=0,  # 填充的值为0
    )
    ...
    group_20_results = df_merged_with_last_20_clicked_items.groupby(["user_id", "date"])[
        "last_k_clicked_items"
    ].apply(check_identical_sequences)
    group_50_results = df_merged_with_last_50_clicked_items.groupby(["user_id", "date"])[
        "last_k_clicked_items"
    ].apply(check_identical_sequences)

    # 找到不匹配的组
    mismatched_20_groups = group_20_results[~group_20_results].index.tolist()
    mismatched_50_groups = group_50_results[~group_50_results].index.tolist()

    # 断言并提供详细的错误信息，如果存在不匹配的序列
    assert (
        len(mismatched_20_groups) == 0
    ), f"Found mismatched sequences for the following user-date pairs: {mismatched_20_groups}"
    assert (
            len(mismatched_50_groups) == 0
    ), f"Found mismatched sequences for the following user-date pairs: {mismatched_50_groups}"

    # 将last_k_clicked_items添加到df_merged
    df_merged["short_clicked_items"] = list(
        df_merged_with_last_20_clicked_items["last_k_clicked_items"].values
    )
    df_merged["long_clicked_items"] = list(
        df_merged_with_last_50_clicked_items["last_k_clicked_items"].values
    )
    print("生成序列mask...")
    df_merged["short_clicked_mask"] = df_merged["short_clicked_items"].apply(
        lambda seq: [1 if item != 0 else 0 for item in seq]
    )
    df_merged["long_clicked_mask"] = df_merged["long_clicked_items"].apply(
        lambda seq: [1 if item != 0 else 0 for item in seq]
    )
    print("计算会话id...")
    ...
    for data_type, data_df in zip(["train", "test"], [train_df, test_df]):
        ...
        for feat_name in data_df.columns:
            if feat_name not in ("short_clicked_items","short_clicked_mask","long_clicked_items","long_clicked_mask",):
    ...
    # =====================================================
    # 新增：保存用户序列数据 user_sequences.pkl
    # 用于Swing、Item2Vec、DSSM、MIND、SDM等召回模型
    # 目前这部分用的数据可能存在数据泄露问题，需要使用训练集的数据构造，待修改
    # =====================================================
    print("保存用户序列数据...")

    # 按user_id聚合，获取每个用户的序列信息
    seq_20_unique = df_merged_with_last_20_clicked_items[
        ["user_id", "date", "last_k_clicked_items"]].drop_duplicates(subset=["user_id", "date"]).drop_duplicates(
        subset=["user_id"])
    seq_50_unique = df_merged_with_last_50_clicked_items[
        ["user_id", "date", "last_k_clicked_items"]].drop_duplicates(subset=["user_id", "date"]).drop_duplicates(
        subset=["user_id"])

    short_seq_dict = dict(zip(seq_20_unique["user_id"], seq_20_unique["last_k_clicked_items"]))
    long_seq_dict = dict(zip(seq_50_unique["user_id"], seq_50_unique["last_k_clicked_items"]))

    all_users = df_merged["user_id"].unique()
    ...
    for uid in all_users:
        user_ids.append(uid)
        short_seq = short_seq_dict.get(uid, [0] * SHORT_LEN)
        short_seqs.append(short_seq)
        short_masks.append([1 if x != 0 else 0 for x in short_seq])
        short_seq_lens.append(sum([1 for x in short_seq if x != 0]))
        long_seq = long_seq_dict.get(uid, [0] * LONG_LEN)
        long_seqs.append(long_seq)
        long_masks.append([1 if x != 0 else 0 for x in long_seq])
        long_seq_lens.append(sum([1 for x in long_seq if x != 0]))

    user_sequences = {
        "user_id": np.array(user_ids),
        "short_seq": np.array(short_seqs),
        "long_seq": np.array(long_seqs),
        "short_mask": np.array(short_masks),
        "long_mask": np.array(long_masks),
        "short_seq_len": np.array(short_seq_lens),
        "long_seq_len": np.array(long_seq_lens),
    }
    ...
    # =====================================================
    # 新增：保存视频信息 video_info.pkl
    # 用于新鲜度召回（包含upload_dt时间戳）
    # =====================================================
    print("保存视频信息...")
    ...
    print("\n预处理完成！")
    print(f"输出文件:")
    print(f"  1. kuairand_train_eval.pkl  - 精排训练/测试数据")
    print(f"  2. kuairand_feature_dict.pkl - 特征字典")
    print(f"  3. user_sequences.pkl       - 用户序列（short_seq/long_seq）")
    print(f"  4. video_info.pkl           - 视频信息（含upload_timestamp）")
